...
const RecommendationDetailPage = () => {
  ...
  const handleAdopt = async () => {
    ...
    setBusy("adopt");
    try {
      const beforeConcentration = (evidence?.simulationLog?.[0] as any)?.concentration ?? null;
      const afterConcentration = (evidence?.simulationLog?.[1] as any)?.concentration ?? null;
      const beforeDrawdown = (evidence?.simulationLog?.[0] as any)?.drawdown ?? null;
      const afterDrawdown = (evidence?.simulationLog?.[1] as any)?.drawdown ?? null;
      const dv = beforeDrawdown !== null && afterDrawdown !== null ? Number(((beforeDrawdown - afterDrawdown) * 100).toFixed(2)) : 0;

      await sb.from("simulations").insert({
        user_id: user.id,
        recommendation_id: rec.id,
        before_snapshot: evidence?.simulationLog?.[0] ?? {},
        after_snapshot: evidence?.simulationLog?.[1] ?? {},
        expected_volatility_change: dv,
        success_probability_delta: dv,
        effective_until: rec.effectiveUntil,
      });
      await updateRecommendationStatus(user.id, rec.id, "simulated");
      await sb.from("decision_logs").insert({ user_id: user.id, recommendation_id: rec.id, action: "simulated", reason: null });
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      await sb.from("decision_logs").insert({ user_id: user.id, recommendation_id: rec.id, action: "rejected", reason });
  ...
  const handleRevoke = async () => {
    ...
    try {
      await sb.from("simulations").update({ active: false, revoked_at: new Date().toISOString() }).eq("user_id", user.id).eq("recommendation_id", rec.id);
      await updateRecommendationStatus(user.id, rec.id, "active");
      await sb.from("decision_logs").insert({ user_id: user.id, recommendation_id: rec.id, action: "revoked", reason: null });
