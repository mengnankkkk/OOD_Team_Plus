    ...
    <div className="mx-auto max-w-4xl">
      ...
      <section className="paper-card p-6 md:p-8">
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          <div className="space-y-2"><Label>称呼</Label><Input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="想让我怎么称呼你" /></div>
          <div className="space-y-2"><Label>年龄</Label><Input type="number" value={age} onChange={(e) => setAge(e.target.value)} placeholder="例如 28" /></div>
          <div className="space-y-2 md:col-span-2"><Label>家庭责任 / 状况</Label><Input value={household} onChange={(e) => setHousehold(e.target.value)} placeholder="例如 单身 / 已婚有孩 / 赡养父母" /></div>
        </div>

        <p className="eyebrow mt-8">现金流与负债</p>
        <div className="mt-5 grid gap-5 md:grid-cols-3">
          <div className="space-y-2"><Label>月度收入（元）</Label><Input type="number" value={monthlyIncome} onChange={(e) => setMonthlyIncome(e.target.value)} placeholder="20000" /></div>
          <div className="space-y-2"><Label>月度必要支出（元）</Label><Input type="number" value={monthlyExpense} onChange={(e) => setMonthlyExpense(e.target.value)} placeholder="10000" /></div>
          <div className="space-y-2"><Label>负债余额（元）</Label><Input type="number" value={liabilities} onChange={(e) => setLiabilities(e.target.value)} placeholder="0" /></div>
        </div>
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          ...
          <div className="space-y-2"><Label>期望的应急金月数</Label><Input type="number" value={emergencyMonths} onChange={(e) => setEmergencyMonths(e.target.value)} placeholder="6" /></div>
          <div className="space-y-2 md:col-span-2"><Label>补充说明（可选）</Label><Textarea rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="例如：受不了 15% 以上的账面回撤 / 喜欢分批投入 / 计划三年后置换房产" /></div>
        </div>
