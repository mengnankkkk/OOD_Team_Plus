...
const DrawdownChart = ({ metrics, loading }: DrawdownChartProps) => {
  ...
  if (loading) {
    return <section className="paper-card p-6 animate-pulse"><div className="h-32 rounded bg-muted" /></section>;
  }
  const dd = drawdown ?? 0;
  const alert = dd > 0.2;
  ...
  return (
    <section className="paper-card p-6">
      ...
      <div className="relative mt-6 h-36 overflow-hidden rounded-md bg-muted/50">
        ...
        {alert && <span className="absolute bottom-4 right-6 rounded bg-card px-2 py-1 text-[11px] shadow-sm">集中度警报触发</span>}
      </div>
