...
const RecommendationCard = ({ rec, onGenerate, generating }: RecommendationCardProps) => {
  ...
  if (!rec) {
    return (
      <section className="paper-card relative overflow-hidden p-6 md:p-8">
        ...
        <div className="mt-6"><Button onClick={onGenerate} disabled={generating} className="h-11 rounded-sm px-6"><Sparkles className="size-4" />{generating ? "Agent 正在协作…" : "生成一轮 Multi-Agent 建议"}</Button></div>
      </section>
    );
  }
  ...
  return (
    <section className="recommendation-card paper-card relative overflow-hidden p-6 md:p-8">
      ...
      <div className="mt-4 rounded-md border border-border bg-muted/30 px-4 py-2 text-xs text-muted-foreground">
        失效条件：{rec.expireCondition}
      </div>
      <div className="mt-7 flex flex-wrap items-center gap-3 border-t border-border pt-6">
        <Button className="h-11 rounded-sm px-6" onClick={() => navigate(`/recommendations/${rec.id}`)}>模拟采纳 <ArrowRight /></Button>
        <Button variant="outline" className="h-11 rounded-sm" onClick={() => navigate(`/recommendations/${rec.id}?tab=evidence`)}>查看证据与反方观点</Button>
