...
const AdvisorTrace = ({ trace, defaultOpen = false }: Props) => {
  ...
  return (
    <div className="mt-3 rounded-md border border-border bg-card">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between gap-3 rounded-md px-3 py-2 text-xs transition-colors hover:bg-muted/50"
      >
        <span className="flex items-center gap-2 text-muted-foreground">
          {open ? <ChevronDown className="size-3.5" /> : <ChevronRight className="size-3.5" />}
          <Brain className="size-3.5 text-primary" />
          <span className="font-medium text-foreground">思维链 · trace {trace.id.slice(-6)}</span>
          <span className="text-muted-foreground">{totalSpans} 步 · {trace.totalMs} ms · {trace.model}</span>
        ...
        <div className="border-t border-border px-3 py-3">
          ...
          <div className="mt-3 rounded-md border border-primary/30 bg-primary/5 px-3 py-2 text-xs">
            <div className="flex items-center gap-2 text-primary">
              ...
              <span className="font-medium">本轮 loop 最终回复</span>
            </div>
