...
export async function updateRecommendationStatus(_userId: string, id: string, status: string, reason?: string): Promise<void> {
  const action = status === "rejected" ? "REJECT"
    : status === "simulated" ? "ACCEPT"
      : status === "active" ? "REVOKE"
        : "DEFER";
  await apiPost(`/api/v1/recommendations/${id}/decisions`, { action, reason: reason?.trim() || undefined });
}
