...
type AuthState = { user: FrontendAuthUser | null; loading: boolean; signIn: (username: string, password: string, locale?: "zh-CN" | "en-US") => Promise<void>; signUp: (username: string, password: string, displayName?: string, locale?: "zh-CN" | "en-US") => Promise<void>; signOut: () => Promise<void>; refresh: () => Promise<void> };
...
export function FrontendAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<AuthState>(() => ({ user, loading, refresh, async signIn(username, password, locale) { const result = await apiPost<{ user: FrontendAuthUser }>("/api/v1/auth/login", { username, password, locale }); setUser(result.user); }, async signUp(username, password, displayName, locale) { const result = await apiPost<{ user: FrontendAuthUser }>("/api/v1/auth/register", { username, password, displayName, locale }); setUser(result.user); }, async signOut() { await apiPost<void>("/api/v1/auth/logout"); setUser(null); } }), [user, loading]);
