...
export function createPortfolioNotifications(userId: string): number {
  ...
  let created = 0;
  for (const snapshotId of activePortfolioSnapshotIds(db, userId)) {
    const view = getPortfolioHoldings(userId, snapshotId);
    if (!view) continue;
    const portfolioId = String(view.portfolioId);
    const metrics = getPortfolioMetrics(userId, snapshotId);
    ...
    if (pnlRate !== null && pnlRate <= -0.08) {
      ...
      created += insertNotification(db, {
        ...
        sourceType: "PORTFOLIO_RISK", sourceId: String(item.instrumentId),
        groupKey: `portfolio:${portfolioId}:holding:${item.instrumentId}:loss`,
        dedupeKey: `${userId}:${portfolioId}:holding-loss:${item.instrumentId}:${urgent ? "urgent" : "important"}:${dataBucket}`, dataAsOf,
    ...
    if (pnlRate !== null && pnlRate >= 0.15) {
      created += insertNotification(db, {
        ...
        sourceType: "PORTFOLIO_GAIN", sourceId: String(item.instrumentId),
        groupKey: `portfolio:${portfolioId}:holding:${item.instrumentId}:gain`,
        dedupeKey: `${userId}:${portfolioId}:holding-gain:${item.instrumentId}:${pnlRate >= 0.3 ? "high" : "normal"}:${dataBucket}`, dataAsOf,
    ...
    if (item.weight >= 0.35) {
      ...
      created += insertNotification(db, {
        ...
        sourceType: "CONCENTRATION_RISK", sourceId: String(item.instrumentId),
        groupKey: `portfolio:${portfolioId}:holding:${item.instrumentId}:concentration`,
        dedupeKey: `${userId}:${portfolioId}:concentration:${item.instrumentId}:${urgent ? "urgent" : "important"}:${dataBucket}`, dataAsOf,
    ...
    if (previousPrice?.gt(0)) {
      ...
      if (move.abs().gte(0.05)) {
        created += insertNotification(db, {
          ...
          sourceType: "MARKET_MOVE", sourceId: String(item.instrumentId),
          groupKey: `portfolio:${portfolioId}:holding:${item.instrumentId}:market-move`,
          dedupeKey: `${userId}:${portfolioId}:market-move:${item.instrumentId}:${move.gte(0) ? "up" : "down"}:${dataBucket}`, dataAsOf,
  ...
  if (metrics && metrics.riskScore >= 70) {
    created += insertNotification(db, {
      ...
      sourceType: "PORTFOLIO_HEALTH", sourceId: String(view.portfolioSnapshotId),
      groupKey: `portfolio:${portfolioId}:risk-score`,
      dedupeKey: `${userId}:${portfolioId}:portfolio-risk:${metrics.riskScore >= 85 ? "urgent" : "important"}:${dataBucket}`, dataAsOf,
      metadata: { portfolioId, rule: "PORTFOLIO_RISK_SCORE", metricValue: metrics.riskScore, threshold: 70, dataAsOf, advisorPrompt: `请基于 ${dataAsOf} 的最新组合快照解释风险评分 ${metrics.riskScore}/100，列出最主要风险、反方证据和三种可模拟的调整方案。` },
    });
  }
  if (String(view.dataQuality).toUpperCase() !== "COMPLETE") {
    created += insertNotification(db, {
      ...
      sourceType: "DATA_QUALITY", sourceId: String(view.portfolioSnapshotId),
      groupKey: `portfolio:${portfolioId}:data-quality`,
      dedupeKey: `${userId}:${portfolioId}:data-quality:${view.portfolioSnapshotId}`, dataAsOf,
      metadata: { portfolioId, rule: "PARTIAL_DATA", dataQuality: view.dataQuality, sourceStatuses: view.sourceStatuses, dataAsOf },
    });
  }
  if (Date.now() - Date.parse(dataAsOf) > STALE_DATA_MS) {
    ...
    created += insertNotification(db, {
      ...
      sourceType: "DATA_FRESHNESS", sourceId: String(view.portfolioSnapshotId),
      groupKey: `portfolio:${portfolioId}:data-freshness`,
      dedupeKey: `${userId}:${portfolioId}:data-stale:${currentBucket}`, dataAsOf,
      metadata: { portfolioId, rule: "STALE_DATA", dataAsOf },
    });
  }
  }
  db.close();
...
function activePortfolioSnapshotIds(
  db: ReturnType<typeof getDatabase>,
  userId: string,
): string[] {
  const rows = db.prepare(`SELECT ps.id FROM portfolio_snapshots ps
    WHERE ps.user_id=?
      AND EXISTS (
    ...
    ORDER BY ps.portfolio_id,ps.id`).all(userId) as Array<{ id: string }>;
