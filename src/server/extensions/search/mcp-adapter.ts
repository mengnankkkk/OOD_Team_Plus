...
export async function searchMCP(query: string, filters: SearchFilters = {}): Promise<SearchResult[]> {
  ...
  if (!endpoint) return [];
  const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query, limit: filters.limit ?? 5 }), signal: AbortSignal.timeout(filters.timeoutMs ?? 8_000) });
