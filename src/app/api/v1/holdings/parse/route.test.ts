...
describe("holding text parsing", () => {
  it("parses a Chinese index position and synchronizes a confirmed ETF holding", async () => {
    const parsed = await POST(authenticatedRequest("http://localhost/api/v1/holdings/parse", {
      method: "POST",
    ...
    const confirmed = await confirm(
      authenticatedRequest(`http://localhost/api/v1/holdings/parse/${parseId}/confirm`, {
        method: "POST",
    ...
    const latest = db.prepare("SELECT id FROM portfolio_snapshots WHERE user_id=? ORDER BY created_at DESC LIMIT 1").get(TEST_USER_ID) as { id: string };
