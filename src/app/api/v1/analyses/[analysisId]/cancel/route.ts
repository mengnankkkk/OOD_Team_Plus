...
export async function POST(request: Request, context: Context) {
  try {
    ...
    return await idempotentApiResponse(request, advisorStore.database, DEMO_USER_ID, "analysis.cancel", { analysisId }, () => ({
      data: new AdvisorService().cancelAnalysis(analysisId),
      status: 200,
    }));
  } catch (error) {
